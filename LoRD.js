...
module.exports = hisoka = async (hisoka, m, chatUpdate, store) => {
    ...
    thumbnail: thumb
        ...
        const replay = (teks) => {
            hisoka.sendMessage(m.chat, { text: teks, contextInfo:{"externalAdReply": {"title": `JOOY BOT`,"body": `تابعني على انستغرام`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": global.tgumb,"sourceUrl": "https://instagram.com/mo_elhouari"}}}, { quoted: lordofc})
        }
...
hisoka.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
	    ...
	    hisoka.sendTextWithMentions(m.chat, `@${roof.p2.split`@`[0]} رفض الدعوى ، تم إلغاء الدعوى`, m)
...
//delete roof[roof.id].waktu
	    hisoka.sendText(m.chat, `تم إرسال الدعوى للدردشة
...
klik https://wa.me/${botNumber.split`@`[0]}`, m, { mentions: [roof.p, roof.p2] })
	    if (!roof.pilih) hisoka.sendText(roof.p, `الرجاء تحديد \n\nBatu🗿\nKertas📄\nGunting✂️`, m)
	    if (!roof.pilih2) hisoka.sendText(roof.p2, `الرجاء تحديد \n\nBatu🗿\nKertas📄\nGunting✂️`, m)
	    roof.waktu_milih = setTimeout(() => {
	    if (!roof.pilih && !roof.pilih2) hisoka.sendText(m.chat, `كلا اللاعبين لا ينوي اللعب,\nSuit dibatalkan`)
	    else if (!roof.pilih || !roof.pilih2) {
	    win = !roof.pilih ? roof.p2 : roof.p
	    hisoka.sendTextWithMentions(m.chat, `@${(roof.pilih ? roof.p2 : roof.p).split`@`[0]} لا تختار بدلة ، تنتهي اللعبة`, m)
	    }
	    delete this.suit[roof.id]
	    return !0
	    }, roof.timeout)
	    ...
	    roof.text = m.text
	    m.reply(`Kamu telah memilih ${m.text} ${!roof.pilih2 ? `\n\nفي انتظار التصويت للخصم` : ''}`)
	    if (!roof.pilih2) hisoka.sendText(roof.p2, '_اختار الخصم_\nSekarang giliran kamu', 0)
	    }
...
break
            ...
            case 'bcgc': case 'bcgroup': {
                ...
                for (let i of anu) {
                    ...
                    let btn = [{
                                urlButton: {
                                    displayText: 'قناتي',
                                    url: 'https://elhouari.tk'
                                }
                            }, {
                                urlButton: {
                                    displayText: 'انستغرامي',
                                    url: 'https://instagram.com/mo_elhouari'
                                }
                            }, {
                                quickReplyButton: {
        ...
        if (!text) throw `Example : ${prefix + command} سحلية الحياة`
...
let template = await generateWAMessageFromContent(m.chat, proto.Message.fromObject({
                listMessage :{
                    ...
                    description: `😄😉`,
                    buttonText: "Menu",
                    footerText: "@@mo_elhouari",
                    listType: "SINGLE_SELECT",
                    sections: [{
								"title": "JOOY BOT",
								"rows": [
									{
										"title": "قسم منشئ البوت",
										"description": "",
										"rowId": `groupmenu`
									},
									{
          ...
          listType: 1
            ...
            const lordser = fs.readFileSync('./src/JOOYBOT.mp3') 
    ...
    const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
                    templateMessage: {
                       ...
                       //     jpegThumbnail: fs.readFileSync('./lib/bobiz.jpg')},
                            hydratedFooterText: `JOOY BOT`,
                            hydratedButtons: [{
                                urlButton: {
                                    displayText: '𝑾𝒉𝒂𝒕𝒔𝑨𝒑𝒑',
                                    url: 'https://wa.me/212646240638'
                                }
                            }, {
                                urlButton: {
...
break
          ...
          case 'menu': case 'help': case '?': case 'بوت':{
                anu = `
 ❃━━━━✤⛩️✤━━━━❃
